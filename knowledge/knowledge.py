...
client = MongoClient(
    "mongodb+srv://admin:admin@cluster0.nwcrbvw.mongodb.net/?retryWrites=true&w=majority")
...
def map_info():
    if request.method == 'POST':  # insert map_info document
        ...
        last_map_document = json.loads(
            collection_map.find().sort({_id: -1}).limit(1))
...
def package_info():
    if request.method == 'POST':  # insert package_info document
        ...
        collection_package.insert_one({"id_package": package_info['id_package'], "source":  package_info['source'],
                                      "destination": package_info['destination'], "priority": package_info['priority']}).inserted_id
    else:  # fetch first package_info document (FIFO)
        first_package_document = json.loads(
            collection_package.find().sort({_id: 1}).limit(1))
        collection_package.remove(first_package_document)
